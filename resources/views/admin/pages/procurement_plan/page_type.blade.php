    ...
    <div class="card mb-3">
        <div class="card-body py-3">
            ...
            <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg" style="margin-top: 5%;">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('ProcurementPlanTypeCreate') }}" method="POST">
                            @csrf
            ...
            <table id="basic-datatables" class="table table-striped table-hover datatable-th">
                ...
                <tbody>
                    @foreach ($ProcurementPlanType as $index => $details)
                        <tr>
                            <td class="text-center">{{ $index + 1 }}</td>
                            <td>{{ $details->type_name }}</td>
                            <td class="text-center">
                                <a href="{{ route('ProcurementPlanShowDetail', $details->id) }}"
                                    class="btn btn-primary btn-sm" title="เพิ่มไฟล์"><i class="bi bi-plus-square"></i></a>
                                ...
                                <form action="{{ route('ProcurementPlanTypeDelete', $details->id) }}" method="POST"
            ...
            @foreach ($ProcurementPlanType as $details)
                <div class="modal fade" id="editModal-{{ $details->id }}" tabindex="-1"
                    ...
                    <div class="modal-dialog modal-lg" style="margin-top: 5%;">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('ProcurementPlanTypeUpdate', $details->id) }}" method="POST">
                                @csrf
                                @method('PUT')
